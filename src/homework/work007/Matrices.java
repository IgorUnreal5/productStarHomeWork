...
public class Matrices {
    ...
    public static class Vector {
        ...
        public Vector add(Vector other) {
            ...
            Vector newVector = new Vector(n);
            for (int i = 0; i < n; i++) {
                newVector.items[i] = this.items[i] + other.items[i];
            }
            return newVector;
        }
        ...
        public Vector scalarMultiply(int scalar) {
            Vector newVector = new Vector(items.length);
            for (int i = 0; i < items.length; i++) {
                newVector.items[i] = this.items[i] * scalar;
            }
            return newVector;
        }
    ...
    public static class Matrix {
        ...
        public Matrix add(Matrix other) {
            Matrix newMatrix = new Matrix(this.nRows, this.nCols);
            for (int i = 0; i < this.nRows; i++) {
                for (int j = 0; j < this.nCols; j++) {
                    newMatrix.rows[i][j] = rows[i][j] + other.rows[i][j];
                }
            }
            return newMatrix;
        }
        ...
        public Matrix multiply(Matrix other) {
            ...
            for (int i = 0; i < other.rows.length; i++) {
                for (int j = 0; j < other.rows.length; j++) {
                    for (int k = 0; k < newMatrix.rows.length; k++) {
                        newMatrix.rows[i][j] += rows[i][k] * other.rows[k][j];
                    }
                }
            }
            return newMatrix;
        ...
        public Matrix scalarMultiply(int scalar) {
            ...
            for (int i = 0; i < this.nRows; i++) {
                for (int j = 0; j < this.nCols; j++) {
                    newMatrix.rows[i][j] = rows[i][j] * scalar;
                }
            }
            return newMatrix;
        }
        ...
        static int det(int[][] rows) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                ans += l * rows[0][i] * det(B);
                l *= (-1);
            }
            return ans;
        }

        private static int[][] GetMinor(Matrix matrix, int column) {
                ...
                for (int j = 0; j <= minorLength; j++) {
                    if (i == 0) {
                        ...
                        if (j == column) {
                            ...
                            minor[i - dI][j - dJ] = matrix.rows[i][j];
                        }
                    }
                }
            }
